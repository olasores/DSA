...
GroceryItemDatabase::GroceryItemDatabase( const std::string & filename )
{
  ...
  GroceryItem gItemsss;
  while (fin >> gItemsss)
  {
    groceryItem.push_back(gItemsss);
  }
...
{
  std::vector<GroceryItem>::iterator it = groceryItem.begin();
  return find(upc, it);
}

// private herlper find function
GroceryItem * GroceryItemDatabase::find(const std::string &upc, std::vector<GroceryItem>::iterator &iter)
{
  if (iter == groceryItem.end())
  {
    return nullptr;
  }
  else if (iter->upcCode() == upc)
  {
    return &(*iter);
  }
  else
