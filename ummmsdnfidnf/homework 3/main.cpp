  ...
  *********************************************************************************************************************************/
  void carefully_move_grocery_items( std::size_t quantity, std::stack<GroceryItem> & broken_cart, std::stack<GroceryItem> & working_cart, std::stack<GroceryItem> & spare_cart )
  {
    ///////////////////////// TO-DO (1) //////////////////////////////
      ...
      if(quantity == 1) {
        GroceryItem temp = broken_cart.top();
        broken_cart.pop();
...
int main( int argc, char * argv[] )
{
  try
  {
    ...
    ///////////////////////// TO-DO (4) //////////////////////////////
      ...
      myCart.push(GroceryItem("milk", "any", "00075457129000", 0.0));
      myCart.push(GroceryItem("rice krispies", "Kellogg's", "00038000291210", 0.0)); 
      myCart.push(GroceryItem("hotdogs", "Applegate Farms" , "00025317533003", 0.0)); 
      myCart.push(GroceryItem("apple pie", "any" , "09073649000493", 0.0)); 
      myCart.push(GroceryItem("bread", "any" , "00835841005255", 0.0)); 
      myCart.push(GroceryItem("eggs", "any", "00688267039317", 0.0)); 
    ...
    ///////////////////////// TO-DO (7) //////////////////////////////
      ...
      GroceryItem tempCart = checkoutCounter.front();
      checkoutCounter.pop();

      const GroceryItem * gitems = worldWideDatabase.find(tempCart.upcCode());
      if(gitems != nullptr) {
        amountDue = amountDue + gitems->price();
        std::cout << std::quoted(gitems->upcCode()) << ", " << std::quoted(gitems->brandName()) << ", " << std::quoted(gitems->productName()) << ", " << gitems->price() << "\n";
      } else {
        std::cout << std::quoted(tempCart.upcCode()) << ", " << std::quoted(tempCart.productName()) << " not found, so today is your lucky day - You get it free! Hooray!" << '\n'; 
      }
