...
std::istream & operator>>( std::istream & stream, GroceryItem & groceryItem )
{
  ///////////////////////// TO-DO (21) //////////////////////////////
    ...
    GroceryItem objectItem; 
    char delimeter = '\0';

    if (stream >> std::ws >> std::quoted(objectItem._upcCode) && stream >> std::ws >> delimeter && delimeter == ',' && 
        stream >> std::ws >> std::quoted(objectItem._brandName) && stream >> std::ws >> delimeter && delimeter == ',' && 
        stream >> std::ws >> std::quoted(objectItem._productName) && stream >> std::ws >> delimeter && delimeter == ',' &&
        stream >> std::ws >>             objectItem._price) 
      {
        groceryItem = std::move(objectItem);
    }
