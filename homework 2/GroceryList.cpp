...
std::size_t GroceryList::find( const GroceryItem & groceryItem ) const
{
  ...
  // locate the grocery item in the list and return the zero based postion of the grocery item

  // CODE TRY ONE
  // const auto *const varrr = std::find(_gList_array.begin(), _gList_array.end(), groceryItem);
  // if(varrr != _gList_array.end()) {
  //   return std::distance(_gList_array.begin(), varrr);
  // }
  // return size();


  // CODE TRY TWO 
  // const auto *varrr = std::find(_gList_array.begin(), _gList_array.end(), groceryItem);
  // const std::size_t index = std::distance(_gList_array.begin(), varrr);
  // return (varrr != _gList_array.end()) ? index : size();


  // CODE TRY THREE
  const auto *varrr = std::find(_gList_array.begin(), _gList_array.begin() + _gList_array_size, groceryItem);
  const std::size_t index = std::distance(_gList_array.begin(), varrr);
  return (varrr != _gList_array.begin() + _gList_array_size) ? index : size();
...
void GroceryList::insert( const GroceryItem & groceryItem, std::size_t offsetFromTop )        // insert provided grocery item at offsetFromTop, which places it before the current grocery item at offsetFromTop
{
  ...
  // discard duplocate items from getting added into the grocery list. 
  ...
  { /**********  Part 1 - Insert into array  ***********************/
    ///////////////////////// TO-DO (4) //////////////////////////////
      ...
      // BREAKIGN DOWN WHAT TO DO
      // insert into array by shifting all the items after the insertion point offsetfromtop to the right
      // using std::shift right
      // make sure there is room in the array for another grocery item 
      ...
      // CODE TRY THREE 
      // std::shift right 
      auto *var = _gList_array.begin() + offsetFromTop;
      std::shift_right(var, _gList_array.end(), 1);
      _gList_array[offsetFromTop] = groceryItem;
      ++_gList_array_size;
      if(_gList_array_size > _gList_array.size()) {
        throw CapacityExceeded_Ex("Capacity exceeded" exception_location);
       ...
       if (offsetFromTop > _gList_array_size) {
        throw InvalidOffset_Ex("Invalid offset for insertion");
    }
  ...
  { /**********  Part 2 - Insert into vector  **********************/
    ...
    // BREAKING DOWN WHAT TO DO
    // convert zero based offset from the top to an iterator by advanicng the list
    // using std::next
    // auto  = std::next(_gList_vector.begin(), offsetFromTop); 
    // or pointer arithmethic 
    auto pos = _gList_vector.begin() + offsetFromTop;

    // insert into a vector using std::vector::insert()
  ...
  { /**********  Part 4 - Insert into singly linked list  **********/
    ...
    // BREAKING DOWN WHAT TO DO
    // convert zero based offset from the top to an iterator by advanicng before being the list
    auto pos = std::next(_gList_sll.before_begin(), offsetFromTop);
    // insert into a sll list 
...
void GroceryList::remove( std::size_t offsetFromTop )
{
  ...
  { /**********  Part 1 - Remove from array  ***********************/
    ...
    // BREAKING DOWN WHAT TO DO 
    // not sure !!
    // using loops and indexes as implemented in vector.hpp

    // CODE TRY 1
    auto *var = _gList_array.begin() + offsetFromTop;
    std::shift_left(var, _gList_array.end(), 1);
    --_gList_array_size;
    ...
    // std::move(_gList_array.begin() + offsetFromTop, _gList_array.end(), _gList_array.begin() + offsetFromTop - 1);
  ...
  { /**********  Part 2 - Remove from vector  **********************/
    ///////////////////////// TO-DO (9) //////////////////////////////
      ...
      // convert zero based offset from the top to an iterator by advanicng the list 
      auto pos = _gList_vector.begin() + offsetFromTop;
      // erase from a vector using std::vector::erase()
  ...
  { /**********  Part 3 - Remove from doubly linked list  **********/
    ///////////////////////// TO-DO (10) //////////////////////////////
      ...
      // convert zero based offset from the top to an iterator by advanicng the list
      auto pos = std::next(_gList_dll.begin(), offsetFromTop);
      // erase from a dll list using std::list::erase()
  ...
  {/**********  Part 4 - Remove from singly linked list  **********/
    ///////////////////////// TO-DO (11) //////////////////////////////
      ...
      // convert zero based offset from the top to an iterator by advanicng the list
      auto pos = std::next(_gList_sll.before_begin(), offsetFromTop);
      // erase from a sll list using std::forward_list::erase_after()
...
void GroceryList::moveToTop( const GroceryItem & groceryItem )
{
  ///////////////////////// TO-DO (12) //////////////////////////////
    ...
    // BREAKING DOWN WHAT TO DO
    // if the item exists use the remove and insert functions to move it to the top
    // if the item does not exist do nothing
    
    // CODE TRY ONE
    // auto compareIndex = find(groceryItem);
...
GroceryList & GroceryList::operator+=( const std::initializer_list<GroceryItem> & rhs )
{
  ///////////////////////// TO-DO (13) //////////////////////////////
    ...
    // BREAKING DOWN WHAT TO DO
    // append the rhs to the bottom of the list by repeatedly inserting to the bottom of the list 
    // using the += operator
    // add the grocery item at the bottom of each container using the function that can insert to bottom 
    

    // CODE TRY ONE
    for (const auto& groceryItem : rhs) {
      // this->insert(groceryItem, Position::BOTTOM);
...
GroceryList & GroceryList::operator+=( const GroceryList & rhs )
{
  ///////////////////////// TO-DO (14) //////////////////////////////
    ...
    // ranged based loop 
    for( const auto& groceryItem : rhs._gList_dll) {
      insert(groceryItem, Position::BOTTOM);
    }

    // for (const auto& groceryItem : rhs._gList_array) {
    //   insert(groceryItem, Position::BOTTOM);
    // }
...
std::weak_ordering GroceryList::operator<=>( GroceryList const & rhs ) const
{
  ...
  ///////////////////////// TO-DO (15) //////////////////////////////
   ...
   /// find the first grocery item in the common extent that is different using three way comparison gItem <=> gItem2 != 0 and return the results or 


    // CODE TRY ONE
  if(auto result = _gList_array <=> rhs._gList_array; result != 0) return result; 
  if(auto result = _gList_vector <=> rhs._gList_vector; result != 0) return result;
  if(auto result = _gList_dll <=> rhs._gList_dll; result != 0) return result; 
  if (auto result = _gList_sll <=> rhs._gList_sll; result != 0) return result;
...
bool GroceryList::operator==( GroceryList const & rhs ) const
{
  ...
  ///////////////////////// TO-DO (16) //////////////////////////////
    ...
    /// walk the list looking for grocery items that don't match
    //// using the == operator 
...
std::size_t GroceryList::gList_sll_size() const
{
  ///////////////////////// TO-DO (17) //////////////////////////////
    ...
    /// BREAKING DOWN WHAT TO DO
    /// calculate the size of the sll list on demand by walking the list from begin to end counting the number of elements visited
    /// using std::distance() function or write your own loop
    // using a loop 
    // for (auto iter = _gList_sll.begin(); iter != _gList_sll.end(); iter++) {
    ...
    return count;
    // std::distance(_gList_sll.begin(), _gList_sll.end());
...
std::istream & operator>>( std::istream & stream, GroceryList & groceryList )
{
  ...
  ///////////////////////// TO-DO (18) //////////////////////////////
    ...
    /// BREAKING DOWN WHAT TO DO
    /// using the operator >> extract until the end of file grocery items from the provided stream 
    // insert them at the bottom of the provided grocery list
    // be sure to extract grocerty items and not individual 
