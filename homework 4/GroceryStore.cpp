...
GroceryStore::GroceryStore( const std::string & persistentInventoryDB )
{
  ...
  ///////////////////////// TO-DO (2) //////////////////////////////
    ...
    // while theres no error and not the end of the file 
    // read the inventory record from the input stream and add the information to the resident inventory database
...
GroceryStore::GroceryItemsSold GroceryStore::ringUpCustomers( const ShoppingCarts & shoppingCarts, std::ostream & receipt )
{
  ...
  ///////////////////////// TO-DO (3) //////////////////////////////
    ...
    // breaking down what to do
    // merge each customers purchased groceries into todays sales

    for(const auto& [customerName, cart] : shoppingCarts) {
      receipt << customerName << "'s shopping cart contains:\n";
      auto purchasedGroceries = ringUpCustomer(cart, receipt);
...
GroceryStore::GroceryItemsSold GroceryStore::ringUpCustomer( const ShoppingCart & shoppingCart, std::ostream & receipt )
{
  ...
  ///////////////////////// TO-DO (4) //////////////////////////////
    ...
    // breaking down what to do 
    // initiaLie the amount due to zero
    // for every gorcery item in the shpping cart
    // if the item i snot found in the wwwg database
    // inidicate on the recepit that it is free of charge otherwise print the item full description on the receipt
    // add the g items price to the amount due. 
    // Decrease the number of items on hand for the item sold
    // add items upc to the list of groceries purchased 
    // print out the total amount due on the recipt 



    double amountDue = 0.0;
    for(const auto & [upc, groceryItem] : shoppingCart) {
      auto databaseIt = worldWideGroceryDatabase.find(upc);
      if(databaseIt != nullptr) {
        receipt << *databaseIt << '\n';
        amountDue += databaseIt->price();
        //add price of GroceryItem to amountDue
        auto inventoryIt = _inventoryDB.find(upc);
        if(inventoryIt != _inventoryDB.end()) {
          --(inventoryIt->second); 
          //add to purchasedGRoceries
          purchasedGroceries.insert(upc);
        }
      }
      else{
        // receipt <<  '"' << upc << '"' << "  not found, the item is free!\n";
        receipt << '"' << upc << "\" (" << groceryItem.productName() <<  ") not found, the item is free!\n";
      }
      
    }
    receipt << "Total amount due: $" << amountDue << '\n';
...
void GroceryStore::reorderItems( GroceryItemsSold & todaysSales, std::ostream & reorderReport )
{
  ...
  for(auto upc : todaysSales) {
   ...
   if(databaseIt == nullptr) {
     ...
     reorderReport << '{' << *databaseIt << '}';
   }

   if(auto gITemms = inventory().find(upc); gITemms == inventory().end()) {
     reorderReport << "\n *** no longer sold in this store and will not be re-ordered\n";
   } else if(gITemms->second < REORDER_THRESHOLD) {
     reorderReport << "\n only " << gITemms->second << "in stock after selling " << todaysSales.count(upc) 
                                 << "unit(s) below threshold ("  << REORDER_THRESHOLD << ") reodering " 
                                 << LOT_COUNT << "units\n";
     gITemms->second += LOT_COUNT;
   }
  } 
  todaysSales.clear();
